...
import type { BlockActivity, BlockingBlock, BlockingScenario, Node, Route, RouteDefinitionAggregate, RoutePattern, Trip, ValidationFinding } from '../domain/types';
...
type Props = { blockingService: BlockingApplication; tripService: TripGenerationApplication; aggregate: RouteDefinitionAggregate; records: ScenarioRecords; routes: Route[]; routeId: string; onRouteChange: (routeId: string) => void; onReload: () => Promise<void>; onError: (error: unknown, fallback?: string) => void };
...
type BlockSortColumn = 'block' | 'trips' | 'pullOut' | 'start' | 'end' | 'pullIn' | 'running' | 'revenue' | 'platform' | 'layover' | 'layoverPercent' | 'deadhead';
type BlockSort = { column: BlockSortColumn; direction: 'ascending' | 'descending' };
type BlockTableTimes = { pullOut?: number; start?: number; end?: number; pullIn?: number };
type BlockTableRow = { block: BlockingBlock; value?: BlockSummary; trips: number; times: BlockTableTimes; layoverPercent?: number };
...
function blockSortValue(row: BlockTableRow, column: BlockSortColumn): string | number | undefined {
  ...
  if (column === 'pullOut') return row.times.pullOut;
  if (column === 'start') return row.times.start;
  if (column === 'end') return row.times.end;
  if (column === 'pullIn') return row.times.pullIn;
...
export function BlockingWorkspace({ blockingService, tripService, aggregate, records, routes, routeId, onRouteChange, onReload, onError }: Props) {
  ...
  const blockingActionsMenu: MenuGroup[] = [
    { label: 'Blocking Scenario', items: [
    ...
    { label: 'Data', items: [{ id: 'export', label: 'Export Blocking CSV (ZIP)', disabled: !blockingScenario || busy, onSelect: () => { void exportCsv(); } }] },
  ];
  ...
  const blockRows: BlockTableRow[] = dayBlocks.map((block) => {
    ...
    const activityTimings = new Map(value?.activityTimings.map((timing) => [timing.activityId, timing]) ?? []);
    const revenueActivities = block.activities.filter((activity) => activity.type === 'revenueTrip');
    const pullOutActivity = block.activities.find((activity) => activity.type === 'pullOut');
    const pullInActivity = block.activities.find((activity) => activity.type === 'pullIn');
    return {
      block,
      value,
      trips: revenueActivities.length,
      times: {
        pullOut: pullOutActivity ? activityTimings.get(pullOutActivity.id)?.startTime : undefined,
        start: revenueActivities[0] ? activityTimings.get(revenueActivities[0].id)?.startTime : undefined,
        end: revenueActivities.at(-1) ? activityTimings.get(revenueActivities.at(-1)!.id)?.endTime : undefined,
        pullIn: pullInActivity ? activityTimings.get(pullInActivity.id)?.endTime : undefined,
      },
      layoverPercent: value && value.revenueHours > 0 ? value.layoverHours / value.revenueHours * 100 : undefined,
    };
  }).sort((left, right) => {
  ...
  return <div className="blocking-workspace" aria-label="Blocking workspace">
    <section className="module-toolbar" aria-label="Blocking controls"><label>Route<select value={routeId} onChange={(event) => onRouteChange(event.target.value)}>{routes.map((route) => <option key={route.id} value={route.id}>{route.shortName ? `${route.shortName} — ${route.name}` : route.name || 'Untitled route'}</option>)}</select></label><label>Trip Profile<select value={tripProfileId} onChange={(event) => { setScenarioSelectionCleared(false); setTripProfileId(event.target.value); }}>{profiles.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Blocking Scenario<select value={blockingScenarioId} disabled={busy} onChange={(event) => { setScenarioSelectionCleared(!event.target.value); setBlockingScenarioId(event.target.value); }}><option value="">No Blocking Scenario</option>{scenarioOptions.map((scenario) => <option key={scenario.id} value={scenario.id}>{scenario.name}</option>)}</select></label><label>Day<select value={serviceDayId} onChange={(event) => setServiceDayId(event.target.value)}>{days.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><MenuButton label="Actions" triggerAriaLabel="Blocking actions" menuLabel="Blocking actions" groups={blockingActionsMenu} disabled={busy} /></section>
    ...
    </div><div className="blocking-lower-grid"><section className="workflow-section" aria-label="Blocks"><div className="section-title"><div><h2>Blocks</h2></div></div><div className="table-scroll data-grid blocking-blocks-table"><table><colgroup><col className="blocking-block-name-column" /><col className="blocking-block-trips-column" /><col className="blocking-block-time-column" /><col className="blocking-block-time-column" /><col className="blocking-block-time-column" /><col className="blocking-block-time-column" /><col className="blocking-block-hours-column" /><col className="blocking-block-hours-column" /><col className="blocking-block-hours-column" /><col className="blocking-block-hours-column" /><col className="blocking-block-percent-column" /><col className="blocking-block-deadhead-column" /></colgroup><thead><tr><BlockSortHeader label="Block" column="block" sort={blockSort} onSort={sortBlocks} /><BlockSortHeader label="Trips" column="trips" sort={blockSort} onSort={sortBlocks} /><BlockSortHeader label="Pull-out" column="pullOut" sort={blockSort} onSort={sortBlocks} /><BlockSortHeader label="Start" column="start" sort={blockSort} onSort={sortBlocks} /><BlockSortHeader label="End" column="end" sort={blockSort} onSort={sortBlocks} /><BlockSortHeader label="Pull-in" column="pullIn" sort={blockSort} onSort={sortBlocks} /><BlockSortHeader label="Running" column="running" sort={blockSort} onSort={sortBlocks} /><BlockSortHeader label="Revenue" column="revenue" sort={blockSort} onSort={sortBlocks} /><BlockSortHeader label="Platform" column="platform" sort={blockSort} onSort={sortBlocks} /><BlockSortHeader label="Layover" column="layover" sort={blockSort} onSort={sortBlocks} /><BlockSortHeader label="Layover %" column="layoverPercent" sort={blockSort} onSort={sortBlocks} /><BlockSortHeader label="Deadhead" column="deadhead" sort={blockSort} onSort={sortBlocks} /></tr></thead><tbody>{blockRows.map(({ block, value, trips: tripCount, times }) => <tr key={block.id} className={block.id === selectedBlockId ? 'selected-block-row' : undefined}><td><button className="link-button" onClick={() => setSelectedBlockId(block.id)}>{block.label}</button></td><td>{tripCount}</td><td>{time(times.pullOut)}</td><td>{time(times.start)}</td><td>{time(times.end)}</td><td>{time(times.pullIn)}</td><td>{hours(value?.runningHours)}</td><td>{hours(value?.revenueHours)}</td><td>{hours(value?.platformHours)}</td><td>{hours(value?.layoverHours)}</td><td>{value ? percentage(value.layoverHours, value.revenueHours) : '—'}</td><td>{hours(value?.deadheadHours)}</td></tr>)}{!blockRows.length && <tr><td colSpan={12} className="table-empty">No Blocks for this service day. Create an empty Block to start.</td></tr>}</tbody><tfoot><tr><th scope="row" colSpan={6}>Total</th><td>{hours(summary?.runningHours)}</td><td>{hours(summary?.revenueHours)}</td><td>{hours(summary?.platformHours)}</td><td>{hours(summary?.layoverHours)}</td><td>{summary ? percentage(summary.layoverHours, summary.revenueHours) : '—'}</td><td>{hours(summary?.deadheadHours)}</td></tr></tfoot></table></div></section><SummaryPanel summary={summary} serviceDayName={day?.name ?? 'Service Day'} /></div></>}
...
function SummaryPanel({ summary, serviceDayName }: { summary: Awaited<ReturnType<BlockingQueries['getScenarioSummary']>>; serviceDayName: string }) { return <section className="workflow-section" aria-label="Blocking summary"><div className="section-title"><div><h2>{serviceDayName} Summary</h2></div></div>{summary ? <dl className="blocking-summary"><div><dt>Revenue hours</dt><dd>{hours(summary.revenueHours)}</dd></div><div><dt>Running time</dt><dd>{hours(summary.runningHours)}</dd></div><div><dt>Platform hours</dt><dd>{hours(summary.platformHours)}</dd></div><div><dt>Deadhead hours</dt><dd>{hours(summary.deadheadHours)}</dd></div><div><dt>Layover hours</dt><dd>{hours(summary.layoverHours)}</dd></div><div><dt>Revenue miles</dt><dd>{miles(summary.revenueMiles)}</dd></div><div><dt>Platform miles</dt><dd>{miles(summary.platformMiles)}</dd></div></dl> : <p className="profile-usage">Select a Blocking Scenario to view its summary.</p>}</section>; }
